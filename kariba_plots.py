...
if __name__ == '__main__':
    ...
    for category, experiments in cat_record['categories'].iteritems():
        ...
        # plot clinical cases
        pp = PlotUtils(best_fits_cat, all_fits_cat, calib_data_cat, viz_root_sweep_dir, category)
        ...
        if not os.path.exists(err_plots_dir):
            print "Creating clinical incidence directory " + str(err_plots_dir)
        ...
        if not os.path.exists(prev_plots_dir):
            print "Creating clinical incidence directory " + str(prev_plots_dir)
